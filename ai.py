import openai
from openai_auth import OPENAI_API_KEY
...
class TutorGPT(ModelBase):
    ...
    def addTopic(self, topic):
        ...
        if self.mode == "learn":
            ...
            self.prompt = "In JSON format, with keys for question, choices, answer: create a multiple choice quiz for a {LEARNING LEVEL} student about {topic}. Make the quiz {self.quizConfiguration[0]} questions long with {self.quizConfiguration[1]} choices each. Make the choices key in a single dictionary format with the keys capitalized and the JSON object a list of dictionaries" # Change the prompt to fit the user's requirements
        else:
            if self.mode == "expand":
