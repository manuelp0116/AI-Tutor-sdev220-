...
model = TutorGPT(subject='math', gradeLevel='college')
...
class UI:
    ...
    def process_request(self, msg):
        if self.checkFields():
            ...
            Student.chatInput = msg
            response=model.complete()
            self.conversationFrame.addMsg(msg)
            print(response)
            self.chatHistory.append(self.conversationFrame)
        else:
            return
...
class Quiz(UI):
    ...
    def submitAnswer(self):
        ...
        self.create_request(Student.msg)
        self.checkAnswer()
    ...
    def checkAnswer(self):
        ...
        if self.answer == self.correct_answer:
            Student.num_correct_answers.append(self.answer)
            messagebox.showinfo("Result", "Correct!")
        else:
            messagebox.showinfo("Result", "Incorrect!")
            Student.incorrect_answers.append(self.answer)
        self.update_score()
