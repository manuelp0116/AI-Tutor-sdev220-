...
# Sample Generator to simulate an AI typing a response in the main chatWindow in the chatFrame
def getMsg():
...
class UI:
    def __init__(self, window):
        ...
        self.start_button = ctk.CTkButton(self.studentFrame_widgets, text="Start Learning", command=self.start_app, width=200)
        self.start_button.grid(row=6, column=1, padx=10, pady=20)

        #####################################################################################################
        "<><><><><><><><><><><><><><><><><><>   Create Quiz Frame   <><><><><><><><><><><><><><><><><><><><>"
        #####################################################################################################
        self.quizFrame = ctk.CTkFrame(window, width=500, height=620, corner_radius=0, fg_color="transparent")
        ...
        self.createQuizBtn = ctk.CTkButton(self.chatFrame, text="Create a Quiz", command=self.buttonEvent('Create Quiz'))
        ...
        "<><><><><><><><><><><><><><><><><><>   Create Chat Frame   <><><><><><><><><><><><><><><><><><><><>"
        #####################################################################################################
        self.chatFrame = ctk.CTkFrame(window, width=400, height=500, corner_radius=0, fg_color="grey20")
