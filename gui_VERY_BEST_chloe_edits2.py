...
from ai_chloe import TutorGPT # The AI class
from dataclasses import dataclass

model = TutorGPT('history', 'college', 'horses in war', 'learn')
...
class UI:
    def __init__(self, window):
        ...
        self.quizBtn = ctk.CTkButton(self.navbarFrame, corner_radius=0, height=40, border_spacing=10, text="Quiz",
                                                      fg_color="transparent", text_color=("gray10", "gray90"), hover_color=("gray70", "gray30"),
                                                      image=self.quizBtn_image, anchor="w", command=lambda: self.navbarEvent('Quiz', self.quizGen_frame))
        ...
        # self.response_box = scrollableFrame()
        ...
        self.chooseQuiz_button = ctk.CTkButton(self.buttonSub_frame, height=40, width=150, text="Take a quiz", font=ctk.CTkFont(size=15, weight="bold"), command=lambda: self.navbarEvent("Quiz", self.quizGen_frame))
        ...
        self.quizGen_header = ctk.CTkLabel(self.quizGen_frame, text="Quiz Generator", font=ctk.CTkFont(size=20, weight="bold"))
        self.quizGen_header.grid(row=0, column=1, columnspan=2, padx=10, pady=10, sticky='n')

        self.quizGen_subheader = ctk.CTkLabel(self.quizGen_frame, text=f"Let's test your knowledge!\n\nPlease select a subject and a study level, then enter a topic. The quiz will be 10 questions long and multiple choice. Click \"Create Quiz\" when you're ready to start.", font=ctk.CTkFont(size=20), wraplength=500)
        ...
        self.quizGen_subframe.grid(row=2, column=1, padx=10, pady=10)

        self.subject_dropdown = ctk.CTkOptionMenu(self.quizGen_subframe, height=40, width=200,  values=subject_list, variable=ctk.StringVar(), font=(ctk.CTkFont(size=15)))
        self.subject_dropdown.set("Choose a subject:")
        self.subject_dropdown.grid(row=0, column=0, padx=10, pady=10)

        self.gradeLevel_dropdown = ctk.CTkOptionMenu(self.quizGen_subframe, height=40, width=200, values=gradeLevel_list, variable=ctk.StringVar(), font=ctk.CTkFont(size=15))
        self.gradeLevel_dropdown.set("Choose a study level:")
        self.gradeLevel_dropdown.grid(row=0, column=1, padx=10, pady=10)

        self.topic_entry = ctk.CTkEntry(self.quizGen_frame, height=40, width=405, font=ctk.CTkFont(size=15), placeholder_text="Enter a topic here", fg_color="transparent")
        self.topic_entry.grid(row=3, column=1, padx=10, pady=10)

        self.createQuiz_btn = ctk.CTkButton(self.quizGen_frame, height=50, width=200, text="Create Quiz", font=ctk.CTkFont(size=20, weight="bold"), command=lambda: self.getAIQuiz(TutorGPT))
        ...
        self.quiz_header = ctk.CTkLabel(self.quiz_frame, text=f"{self.subject_dropdown.get()} Quiz", font=ctk.CTkFont(size=20, weight="bold"))
    ...
    def getAIQuiz(self, model: TutorGPT):
        model = TutorGPT(mode="quiz", subject=self.subject_dropdown.get(), gradeLevel=self.gradeLevel_dropdown.get(), topic=self.topic_entry.get())
        model.quizMode()
    ...
    def createQuiz(self):
        self.quizContainerFrame = ctk.CTkFrame(self.quizGen_frame, corner_radius=0, fg_color="transparent")
