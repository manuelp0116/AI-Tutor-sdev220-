...
openai.api_key = "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"
        ...
        def wrapper(*args, **kwargs):
            for i in range(max_retries):
                try:
                    ...
                    if i == max_retries:
                        print(
                            "Insure you are connected to the internet and try again\n"
                        )
...
class ModelBase:
    ...
    def complete(self, temperature=0.8, top_p=1, stream=True):
        ...
        self.chat["messages"].append(
            msg
        )  # append the dictionary to the inner list in chat
        ...
        if stream:  # if the user wants to stream their response
            ...
            self.logCompletion(
                collectedMessages
            )  # Add the AI's response to message history
...
#             self.logCompletion(completion)  # Add the AI's response to message history
    ...
    def logCompletion(self, messages):
        ...
        for message in messages:
            if (
                "role" in message
            ):  # If the "role" dictionary is found, place it in the result dictionary
        ...
        ResponseDict.update(
            collectedResponse[0]
        )  # Update the responseDict with the first dictionary in the collectedResponse list

        # loop over the dictionaries in the collectedResponse list
        for response in collectedResponse:
            if (
                "content" in response
            ):  # if there is a "content" key contained within the response dictionary
        ...
        resultDict.update(
            ResponseDict
        )  # concatenate the response dictionary into the resulting dictionary to create the final product

        self.history.append(
            resultDict
        )  # save the resulting message dictionary to history
    ...
    def store(self, message: dict[str, str] | str, *, role="user"):
        if isinstance(message, dict):  # if the message is a dictionary
            ...
            if role in (
                "user",
                "assistant",
            ):  # if the role is either "user" or "assistant"
                messageDict = {
                    "role": role,
                    "content": message,
                ...
                print(
                    f'Role must be either "user" or "assistant". You entered {role}.'
    ...
    def storeAt(self, index, message: dict[str, str] | str, role="user"):
        if isinstance(message, dict):  # if the message is a dictionary
            self.chat["messages"].insert(
                index - 1, message
            )  # insert the message at the position specified

        elif isinstance(message, str):  # if the message is a string
            if role in (
                "user",
                "assistant",
            ):  # if the string is either "user" or "assistant"
                messageDict = {
                    "role": role,
                    "content": message,
                }  # construct the message dictionary
    ...
    def modify(self, index, newMessage: dict[str, str] | str, role=""):
        MAX_INDEX = len(
            self.chat["messages"]
        )  # the amount of messages in the chat - a constant variable
        RESTRICTED_NUMS = [
            1,
            (MAX_INDEX * -1) - 1,
        ]  # These are the indexes of the AI instructions that shouldn't be modified - a constant variable

        if isinstance(newMessage, dict):
            if (
                index > MAX_INDEX
            ):  # if the index is more than the amount of messages in the chat
                print(f"The chat history is not more than {MAX_INDEX} messages long!")
            else:
                for i, dicts in enumerate(
                    self.chat["messages"]
                ):  # enumerate through the dictionaries to get the dictionary and the index
                    if i == index:  # if i equals the index input by the user
                        if i not in RESTRICTED_NUMS:
                            self.chat["messages"].remove(
                                dicts
                            )  # remove the dictionary at that location
                            self.chat["messages"].insert(
                                index, newMessage
                            )  # insert the new message
                        else:
                            print(
                                "The index you have chosen is the AI instructions. You cannot delete this index. Choose another index"
                            )
                    ...
                    if i == index:  # if i equals the index input by the user
                        if i not in RESTRICTED_NUMS:  # if the index is not restricted
                            if role == "":  # if the role string has not been set
                                temp = self.chat["messages"].pop(
                                    i
                                )  # remove the dictionary at that location and grab a temporary version of it
                                messageDict = {
                                    "role": temp["role"],
                                    "content": newMessage,
                                }  # construct the message dictionary
    ...
    def delete(self, index):
        ...
        RESTRICTED_NUMS = [
            ...
            (MAX_INDEX * -1) - 2,
            (MAX_INDEX * -1) - 1,
        ]  # These are the indexes of the AI instructions that shouldn't be deleted - a constant variable
        ...
        if (
            ...
            for i, dicts in enumerate(
                ...
                if (
                    i == index
                ):  # if i equals the index input by the user plus the offset
                    if i not in RESTRICTED_NUMS:
                        self.chat["messages"].remove(
                            dicts
                        )  # remove the dictionary at that location
...
class InstructionsManager:
    ...
    def __init__(self, subject, gradeLevel):
        self.rules = [
            {
                "Rule": "Don't generate content that isn't based on the subject",
                "Sub Rules": [],
            },
            {
                "Rule": "Don't generate content that's not at the grade level",
                "Sub Rules": [],
            },
            {
                "Rule": "Don't generate any content that isn't relevant",
                "Sub Rules": [
                    "a. For example, Alien Planets have nothing to do with Health. Pulleys and Levers have nothing to do with Programming"
                ],
            },
            {
                "Rule": "Tell the user what rule an invalid prompt violates",
                "Sub Rules": [
    ...
    def removeRule(self, index):
        ...
        if index < 0:  # check for a negative number
            ...
            for i, rule in enumerate(self.rules):
                if (
                    -i - 1 == index
                ):  # if the negative of the index matches the negative index from the parameter
...
class TutorGPT(ModelBase):
    def __init__(self, subject, gradeLevel, topic, mode):
        ...
        self.storeAt(
            3,
            "I understand. I will make sure that I enforce all four rules in the way you described.",
            "assistant",
        )
    ...
    def learnMode(self, topic):
        ...
        if self.mode == "learn":
            ...
            print(
                "Creating a quiz based on a topic meant for learning is not helpful to the student, use quizMode()"
            )
        else:
            if self.mode == "expand":
                print(
                    "Add topic reqiures a topic name, not a topic description. Use excerptMode()"
                )
            else:
    ...
    def excerptMode(self, excerpt):
        ...
        if self.mode == "expand":
            ...
            print(
                "Excerpt mode is for excerpts taken from textbooks or other source material that the student needs help understanding, not for learning about a topic. Use learnMode()"
            )
        elif self.mode == "quiz":
            print(
                "creating a quiz based on an excerpt of text taken from a textbook is not helpful to the student, use quizMode()"
            )
